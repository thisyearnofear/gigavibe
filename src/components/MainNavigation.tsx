...
export default function MainNavigation() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("gigavibe-navigate", handleNavigationEvent as EventListener);
  }, []);
  ...
  const renderScreen = () => {
    ...
    if (isChallengeActive && currentChallenge) {
      return <ChallengeFlow challenge={currentChallenge} onComplete={handleChallengeComplete} onCancel={handleChallengeCancel} />;
    }

    switch (activeScreen) {
      case "home":
        return (
          <Container maxWidth="xl" className={`space-y-${isMobile ? '8' : '12'}`}>
            <Section centerTitle title={<>Welcome back{userInfo?.display_name ? `, ${userInfo.display_name}` : ""}!</>} subtitle="Ready to showcase your vocal talents?" />
            <Section centerTitle>
              <ChallengeDiscovery onChallengeSelect={handleChallengeSelect} onViewAllChallenges={() => {}} maxItems={isMobile ? 3 : 5} />
            </Section>
            <Section centerTitle>
              <motion.div
