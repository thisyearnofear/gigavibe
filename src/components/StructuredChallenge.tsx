...
interface StructuredChallengeProps {
  onComplete: (accuracy: number, recording: string) => void;
}

export default function StructuredChallenge({ onComplete }: StructuredChallengeProps) {
  const { pitchData, isListening, startListening, stopListening } = usePitchDetection();
  const [phase, setPhase] = useState<'ready' | 'countdown' | 'singing' | 'complete'>('ready');
  ...
  const totalDuration = challengeNotes[challengeNotes.length - 1].startTime + challengeNotes[challengeNotes.length - 1].duration;
  ...
  const startChallenge = async () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      const currentNote = challengeNotes.findIndex(note => 
  ...
  const completeChallenge = () => {
    ...
    const avgAccuracy = finalAccuracies.reduce((sum, acc) => sum + acc, 0) / finalAccuracies.length;
    
    setPhase('complete');
    onComplete(Math.round(avgAccuracy), 'mock_recording_url');
  };
  ...
  const getProgressPercentage = () => ((totalDuration - timeRemaining) / totalDuration) * 100;
                  ...
                  {challengeNotes.map((note, index) => (
                    <div key={index} className="text-center">
                      <div className="w-12 h-12 bg-purple-500/30 rounded-full flex items-center justify-center mb-2">
                        <span className="font-bold">{note.note}{note.octave}</span>
                      </div>
                      <div className="text-xs text-gray-400">{note.duration}s</div>
                    </div>
                  ))}
                </div>
                <p className="text-gray-300 text-sm">
                  Sing each note clearly for {challengeNotes[0].duration} seconds. Follow the progression!
                </p>
              </div>
          ...
          {phase === 'singing' && (
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-3xl p-8 mb-6">
                ...
                >
                  {getCurrentNote()?.note}{getCurrentNote()?.octave}
                </motion.div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-gray-300">
                    You: {pitchData.frequency > 0 ? `${pitchData.note}${pitchData.octave} (${pitchData.frequency.toFixed(1)} Hz)` : 'Listening...'}
                  </div>
                </div>
              </div>
                ...
                {challengeNotes.map((note, index) => (
                  <div
                    ...
                    className={`w-3 h-3 rounded-full transition-colors duration-300 ${
                      index < currentNoteIndex ? 'bg-green-500' :
                      index === currentNoteIndex ? 'bg-purple-500' :
                      'bg-gray-600'
                    }`}
                  />
                ))}
          ...
          {phase === 'complete' && (
            ...
            >
              ...
              <div className="flex gap-4">
                ...
                <motion.button
                  onClick={() => onComplete(accuracy, 'mock_recording')}
