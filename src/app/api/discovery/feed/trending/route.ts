...
// Sample data for the "Trending" feed
const mockPerformances: RealityCheckResult[] = [
  {
    ...
    farcasterData: {
      ...
      likes: 203, // Highest likes
      recasts: 89, // Highest recasts
      replies: 54
    }
  },
  {
    ...
    farcasterData: {
      ...
      likes: 145, // Second highest likes
      recasts: 67, // Second highest recasts
      replies: 32
    }
  },
  {
  ...
  },
  {
    ...
    shareCount: 156,
    timestamp: new Date('2025-08-30T10:30:00Z'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 1. Fetch trending performances from a database
    // 2. Apply trending algorithm (high engagement in the last 24 hours)
    // 3. Apply pagination (offset/limit)
    
    // For now, we'll use mock data with simple pagination
    // Note: Our mock data is already sorted by engagement metrics (shareCount, likes, recasts)
    ...
    console.error('Error in trending feed API:', error);
    return NextResponse.json(
      { error: 'Failed to retrieve trending feed' },
      { status: 500 }
    );
  }
}